...
- 디스크: 전체 물리 디스크 읽기(R)·쓰기(W) 속도 차트, 로컬 볼륨별 사용/전체 용량.
  볼륨이 많으면 2개씩 6초마다 순환한다. PC 편집기 표에서는 전체 목록을 확인한다.
  디스크 I/O 차트는 활성 시간(%)이 아닌 처리량이다. 네트워크 드라이브와 빈 매체는 제외한다.
...
카드 위치·CPU/메모리 사용률 글자 크기·차트 색상·밝기 편집과 저장본 불러오기를 지원한다.
