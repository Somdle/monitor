...
| Network | 선택한 어댑터의 수신(↓)·송신(↑) 속도, 최근 1분 사용량 | 색 선: 수신 / 흰 선: 송신 |
...
디스크 카드는 용량이나 활성 시간(%)을 표시하지 않습니다. 네트워크는 어댑터 하나만 측정하며,
VPN·가상 어댑터가 자동 선택되면 편집기의 **네트워크 어댑터**에서 원하는 항목을 지정하세요.

<details>
<summary>네트워크 자동 선택과 1분 사용량 계산 방식</summary>

자동 선택은 활성 어댑터 중 OS 누적 송수신량이 가장 큰 하나를 고르고, 활성 상태가 유지되는 동안 계속 사용합니다.
여러 어댑터의 트래픽을 합산하지 않습니다.
