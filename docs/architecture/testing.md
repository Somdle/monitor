...
test_sensors.py는 psutil/시간 외부 경계만 대체하여 다중 볼륨, 실제 메모리, 시간 기반 속도,
누적량, 카운터 초기화, 어댑터/디스크 변경, 부분 실패, 60초 이력 제한을 검증한다.
test_render.py는 차트 픽셀 변화, 볼륨 페이지 전환 및 카드 밖 침범 방지를 검증한다.
